...
def encoder_rnn(rnn_input, rnn_size, num_of_layers, keep_prob, sequence_length):
    ...
    output, state = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell,
                                                    cell_bw=cell,
...
def seq2seq_model(inputs, targets, keep_prob, batch_size, sequence_length, answers_num_words, questions_num_words,
    ...
    # build decoder rnn
    _batch_targets = batch_targets(targets, batch_size, sos_id)
    decoder_embeddings_matrix = tf.Variable(tf.random_uniform([questions_num_words + 1, decoder_embedding_size], 0, 1))
    decoder_embedded_input = tf.nn.embedding_lookup(decoder_embeddings_matrix, _batch_targets)

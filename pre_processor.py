...
def read_files():
    logging.info('Start importing data sets')
    ...
    logging.info('Data sets are imported')
...
def clean_data(lines, conversations, contractions):
    logging.info('Building internal data structure from the data set')
    ...
    logging.info('Start cleaning questions and answers')
...
def generate_embedding(clean_questions, clean_answers):
    ...
    logging.info('Selecting top {0}% words out of total {1} words'.format(frequent_threshold * 100, _len))
    selected_words = frequent_distribution.most_common(int(_len * frequent_threshold))

    # Create word2id mapping - Word embedding
    logging.info('Creating word2id for the selected words')
    ...
    logging.info('Creating id2word inverse mapping')
    id2word = {idx: word for word, idx in word2id.items()}

    # Convert words in questions/answers to idx based on word2id
    logging.info('Converting sentence in to [ids] based on word2id mapping')
    ...
    logging.info('Start sorting questions based on the word counts (=length)')
    sorted_questions2id = []
    sorted_answers2id = []
